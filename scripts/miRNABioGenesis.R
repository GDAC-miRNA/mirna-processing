...
{
  print(index)
  bg_GEXP_CNVR_MAF$Variant_Classification[bg_GEXP_CNVR_MAF$SampleBarcode==bgMAF$Tumor_SampleBarcode[index] & bg_GEXP_CNVR_MAF$Symbol==bgMAF$Hugo_Symbol[index]] = bgMAF$Variant_Classification[index]
}
...
bg_FuncAlt = bg_GEXP_CNVR_MAF[,c('SampleBarcode','Study.x','Symbol','normalized_count','IsAltered')]
...
ggplot(bg_FuncAlt[bg_FuncAlt$Symbol=="SMAD4",], aes(IsAltered,log2_count,fill=IsAltered)) + 
  geom_boxplot() + 
...
uniqSampleStudyList = unique(bg_GEXP_CNVR_MAF[,c('SampleBarcode','Study.x')])

...
#mirna id, mirna accession, coordinates, sequence, mature coordinates, mature MIMAT, alternate mature coordinates, alt mature MIMAT
my @mirnas;
...
my $sth = $dbh_mirbase->prepare("
	SELECT m.mirna_id, m.mirna_acc, c.xsome, c.contig_start, c.contig_end, c.strand, m.sequence, ma.mature_name, mature_from, mature_to, ma.mature_acc
	FROM mirna m
	JOIN mirna_chromosome_build c ON m.auto_mirna = c.auto_mirna
	JOIN mirna_pre_mature p ON m.auto_mirna = p.auto_mirna
	JOIN mirna_mature ma ON p.auto_mature = ma.auto_mature
	WHERE m.auto_species = '$species_code'
	ORDER BY m.mirna_id, c.xsome, c.contig_start, c.contig_end, ma.mature_name
");
...
while ($sth->fetchrow_arrayref()) {
	...
	if ($mirna ne $prev_mirna) {
		...
		# we have a new MIMAT info to add to the mirnas array
		# This is where we check if we have found the star stand
		if (index($mature, "-3p" == -1)) {
			#this is the 5p strand
			...
			# If we found a 3p strand, we add the coordinates and accession to 
			# the end of the mirnas array 
